...
export default function App() {
  ...
  return (
    <div className="min-h-screen paper-grid py-6 sm:py-10 px-4 sm:px-6 relative selection:bg-[#1854F2] selection:text-white">
      ...
      {/* 헤더 바 */}
      <header className="max-w-6xl mx-auto mb-8 space-y-4">
        ...
        {/* 🌟 사이트 진입 시 2대 메인 선택 버튼 (신청하기 vs 활동 사진 보기) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => setCurrentTab('form')}
            className={`p-4 rounded-3xl border-2 font-black text-sm sm:text-base flex items-center justify-between transition-all duration-200 shadow-xs cursor-pointer ${
              currentTab === 'form'
                ? 'bg-[#1854F2] border-[#1854F2] text-white shadow-md scale-[1.01]'
                : 'bg-white border-slate-900/15 text-slate-700 hover:border-slate-400 hover:bg-slate-50'
            }`}
          >
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-2xl flex items-center justify-center font-bold text-sm ${currentTab === 'form' ? 'bg-white/20 text-white' : 'bg-blue-50 text-[#1854F2]'}`}>
                📝
              </div>
              <div className="text-left">
                <div className="leading-tight">13기 동아리 신청하기</div>
                <div className={`text-[11px] font-medium ${currentTab === 'form' ? 'text-white/80' : 'text-slate-400'}`}>
                  실시간 크루 패스와 함께 바로 작성
                </div>
              </div>
            </div>
            {currentTab === 'form' && (
              <span className="text-xs bg-white text-[#1854F2] px-2.5 py-1 rounded-xl font-bold">
                작성 중
              </span>
            )}
          </button>
          ...
          <button
            ...
            className={`p-4 rounded-3xl border-2 font-black text-sm sm:text-base flex items-center justify-between transition-all duration-200 shadow-xs cursor-pointer ${
              currentTab === 'gallery'
                ? 'bg-rose-500 border-rose-600 text-white shadow-md scale-[1.01]'
                : 'bg-white border-slate-900/15 text-slate-700 hover:border-slate-400 hover:bg-slate-50'
            }`}
          >
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-2xl flex items-center justify-center font-bold text-sm ${currentTab === 'gallery' ? 'bg-white/20 text-white' : 'bg-rose-50 text-rose-500]'}`}>
                📸
              </div>
              <div className="text-left">
                ...
                <div className={`text-[11px] font-medium ${currentTab === 'gallery' ? 'text-white/80' : 'text-slate-400'}`}>
                  피크닉, 오락실, 보드게임, 사격 등
                </div>
              </div>
            </div>
            {currentTab === 'gallery' && (
              <span className="text-xs bg-white text-rose-600 px-2.5 py-1 rounded-xl font-bold">
                보는 중
              </span>
            )}
          </button>
      ...
      {/* 🌟 탭에 따른 메인 콘텐츠 분기 */}
      {currentTab === 'gallery' ? (
        <main className="max-w-6xl mx-auto animate-in fade-in duration-300">
          <ActivityGallery onGoToForm={() => setCurrentTab('form')} />
        </main>
      ) : (
        /* 메인 폼 콘텐츠 영역: 폼(좌) + 실시간 크루 패스(우) */
        <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-in fade-in duration-300">
          {/* 좌측 폼 본문 */}
      ...
      {/* 모바일 하단 플로팅 패스 보기 버튼 (폼 작성 모드일 때만 표시) */}
      {currentTab === 'form' && (
