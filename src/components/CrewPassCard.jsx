...
export default function CrewPassCard({ formData, isModal = false }) {
  ...
  return (
    <div className={`relative bg-white rounded-3xl polaroid-shadow border-2 border-[#1E293B] overflow-hidden transition-all duration-300 ${isModal ? 'max-w-md w-full mx-auto' : ''}`}>
      ...
      {/* 티켓 상단 헤더 */}
      <div 
        className="p-4 sm:p-5 text-white relative transition-colors duration-300"
        style={{
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <Ticket className="w-4 h-4 sm:w-5 sm:h-5 text-white/90" />
            <span className="font-marker tracking-wider text-[11px] sm:text-xs uppercase text-white/90">Official Member Pass</span>
          </div>
          <span className="bg-black/20 text-white text-[10px] sm:text-[11px] font-bold px-2 py-0.5 rounded-full backdrop-blur-xs font-outfit">
            VOL. 13
          </span>
        </div>

        <div className="mt-2.5 sm:mt-3">
          <h2 className="text-xl sm:text-2xl font-black tracking-tight font-outfit uppercase drop-shadow-xs flex items-center gap-2">
            ALL DAY FRIENDS
      ...
      {/* 티켓 본문 */}
      <div className="p-4 sm:p-6 pt-5 sm:pt-7 space-y-4 sm:space-y-5 bg-[#FCFAF7]">
        {/* 지원자 기본 인포 */}
        <div className="flex items-center justify-between border-b border-dashed border-slate-300 pb-3 sm:pb-4 gap-2">
          <div className="min-w-0 flex-1">
            <div className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wide">Applicant</div>
            <div className="text-lg sm:text-xl font-extrabold text-slate-900 flex flex-wrap items-center gap-1.5 mt-0.5">
              <span className="truncate">{formData.name || '예비 13기 크루'}</span>
              {formData.gender && (
                <span className="text-[11px] sm:text-xs font-semibold px-2 py-0.5 bg-slate-100 rounded-md text-slate-600 shrink-0">
                  {formData.gender} {formData.age ? `· ${formData.age}세` : ''}
                </span>
              )}
            </div>
            <div className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-1 font-medium">
          ...
          <div className="w-13 h-13 sm:w-14 sm:h-14 rounded-2xl bg-amber-50 border-2 border-dashed border-amber-300 flex flex-col items-center justify-center text-center p-1 relative rotate-2 shrink-0">
            <span className="text-[8.5px] sm:text-[9px] font-bold text-amber-700 leading-none">활동권역</span>
        ...
        {/* 선택 테마 카드 */}
        <div>
          <div className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-1.5 flex items-center gap-1">
            <Compass className="w-3.5 h-3.5" />
            1순위 희망 테마
          </div>
          {selectedTheme ? (
            <div className="p-3 sm:p-3.5 rounded-2xl border-2 border-slate-900 bg-white shadow-xs relative overflow-hidden">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[11px] sm:text-xs font-bold px-2 py-0.5 rounded-md bg-slate-100 text-slate-700">
                    {selectedTheme.badge}
                  </span>
                  <div className="font-marker text-base sm:text-lg text-slate-900 mt-1 tracking-wide">
                    {selectedTheme.enTitle}
                  </div>
                </div>
                {currentLevel && (
                  <span className="text-[11px] sm:text-xs font-extrabold text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-xl shrink-0">
            ...
            <div className="p-3 sm:p-3.5 rounded-2xl border-2 border-dashed border-slate-300 text-center text-xs text-slate-400">
